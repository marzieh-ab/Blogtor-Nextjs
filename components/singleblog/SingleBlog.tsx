...
import {  blogs} from "../../pages/allblogs/index"
...
const  SingleBlog:FC<Iprops>=({data}) =>{
  ...
  useEffect(() => {
      fetch(`http://localhost:4000/blog/single-blog/${data._id}`, {
        method: "GET",
    ...
    const submitComment =() =>{
      fetch('http://localhost:4000/comment/submit',{
        ...
        .then(data => {
          ...
          getAllComment()
          window.location.assign(`http://localhost:3000/allblogs`)
    ...
    const getAllComment=()=>{
      ...
      fetch(`http://localhost:4000/comment/by-blog/${data._id}`,{
        method:'GET',
        headers: { 'Content-Type': 'application/json' },
    ...
    const changeRate=(rating:number)=>{
...
<div  className="border-[1px]  rounded-sm  pl-20 mb-5">
        <div  className="my-2">
            <img src={data.imgurl} alt=""     className='border-[1px]  rounded-[50%]  w-[200px]  h-[200px]'/>
      ...
      <span>({data.rateCount})</span>
        ...
        <h1 className='my-2 mx-10  text-2xl'>{data.title}</h1>

        <div className='flex space-x-2    my-2  mx-10 '>
        <div className='font-light  text-justify'  dangerouslySetInnerHTML={{ __html: data.content }}></div>
        </div>
...
(
  <div>
  <input   onChange={(e: React.ChangeEvent<HTMLInputElement>)=>setComment(e.target.value)}  type="text" placeholder='Enter Your Comment' className='bg-[#eee]  pl-2 outline-none rounded-lg  block md:w-[79vw] md:h-[3vw]  h-[50px]  w-[100%]'></input>
